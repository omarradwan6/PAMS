...
class MyTeam extends React.Component {
    ...
    getEmpList() {
        axios.get(`./EMP_DATA/EmpList/${this.props.empID}`)
            .then((response) => {
        ...
        return (
            <div className='container-fluid'>
                        ...
                        {this.state.employees.map((emp, index) => {
                            return (
                                <tr style={this.state.selectedRow === index ? { backgroundColor: "#91AEC1", color: "white" } : {}} onClick={(event) => this.selectRow(event, index, emp.ID, emp.Status, emp.NAME, emp.NETWORKID)}>
                                    ...
                                    <td class={actionStates.includes(emp.Status)?"font-weight-bold":""}>{actionStates.includes(emp.Status) ? "Your Action Required" : "No Action Required"}</td>
                ...
                <div className='d-flex justify-content-center'>
                    <Link onClick={this.sendEmail} className={`btn btn-lg pBackground m-2 ${this.state.selectedStatus === null || this.state.selectedStatus === "Not Created" ? "disabled" : ""}`}
                        to={{
                            pathname: `/Supervisor/EmployeeObjectives/${this.state.selectedID}/`,
                        ...
                        Show PAMS</Link>
                    <Link className={`btn btn-lg pBackground m-2 ${actionStates.includes(this.state.selectedStatus) ? "" : "disabled"}`}
                        to={{
                            pathname: `/Supervisor/EmployeeObjectives/${this.state.selectedID}/`,
                            EmpData: { EmpName: this.state.selectedEmpName, EmpID: this.state.selectedID, EmpPmsStatus: this.state.selectedStatus, EmpMail: this.state.selectedMail },
                            action:"action"
